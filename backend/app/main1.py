...
# 添加当前目录到路径，以便导入 agent_stream
sys.path.append(str(Path(__file__).parent))

from agent_stream import StreamingAgent
...
async def websocket_chat(websocket: WebSocket):
    ...
    # 创建 Agent 实例（每个会话独立）
    agent = StreamingAgent(session_id=session_id, user_id=user_id)
        # 定义发送消息的异步函数
    ...
    try:
        while True:
            ...
            #     send_callback=lambda msg: manager.send_message(session_id, msg)
            # )
            await agent.process_message(
                user_message=user_message,
                send_callback=send_to_client  # ← 修改这里
            )
